...
x2_space = np.linspace(0, 10000, 10000)


def linear_model(reg, x):
    return reg.intercept_ + x * reg.coef_[0]
#
#
# X, Y = np.meshgrid(x1_space, x2_space)

Y = linear_model(reg, X)
...
plt.xlabel(r'$x_2$')
plt.ylabel(r'$y$')
plt.title('Single Regression \"Best Fit Line\" with Scatter Plot')

plt.ylim(-0.2, 1.2)
plt.scatter(x_2, y, c=cs)
plt.colorbar(format=FuncFormatter(get_label))
plt.plot(X, Y, color=ColorConverter().to_rgba('red', alpha=0.8), aa=True)
